document.addEventListener("DOMContentLoaded", () => {
    ...
    function showTooltip(target) {
        ...
        tooltipTimer = window.setTimeout(() => {
            ...
            if (footnoteContent) {
                ...
                tooltipContent = tooltipContent.replace(/^(<br\s*\/?>)*|(<br\s*\/?>)*$/g, "");
    ...
    document.addEventListener("click", (e) => {
        const target = e.target.closest('sup[id^="fnref"] a, .footnote-backref');
        if (target) {
    ...
    document.addEventListener("mouseover", (e) => {
        if (isMobile)
            return;
        const target = e.target.closest('sup[id^="fnref"] a');
        if (target) {
            showTooltip(target);
        }
    });
    document.addEventListener("mouseout", (e) => {
        ...
        if (target) {
            ...
            if (e.clientX < rect.left - MOUSE_TOLERANCE ||
